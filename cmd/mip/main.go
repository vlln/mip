...
func runPull(args []string) int {
	...
	if err := fs.Parse(moveFlagsFirst(args, map[string]bool{
		...
		"--config": true, "--engine": true, "--platform": true, "--timeout": true, "--pull-timeout": true, "--concurrency": true,
	})); err != nil {
		return exitGeneralError
	}
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: mip pull IMAGE [--engine docker|podman|nerdctl] [--dry-run] [--platform PLATFORM] [--no-verify-digest] [--json]")
		return exitGeneralError
	}
	...
	start := time.Now()
	if err := runner.Pull(context.Background(), selected.Image, engine.PullOptions{
		Platform: *platform,
		Timeout:  *pullTimeout,
		Stdout:   os.Stderr,
		Stderr:   os.Stderr,
	...
	verified := false
	if !*noVerifyDigest && selected.Digest != "" {
		repoDigests, err := runner.RepoDigests(context.Background(), selected.Image)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%v\n", err)
			return exitDigestMismatch
		}
		expectedDigests := verificationDigests(selected)
		if !hasAnyDigest(repoDigests, expectedDigests) {
			fmt.Fprintf(os.Stderr, "digest verification failed: expected one of %v, repo digests: %v\n", expectedDigests, repoDigests)
			return exitDigestMismatch
		}
		verified = true
	}
	...
	retagged := false
	if !*noRetag && selected.Image != image.String() {
		if err := runner.Tag(context.Background(), selected.Image, image.String()); err != nil {
			fmt.Fprintf(os.Stderr, "%v\n", err)
			return exitPullFailed
		}
		retagged = true
		_ = runner.Remove(context.Background(), selected.Image)
	}
...
func selectCandidate(ctx context.Context, profiles []registry.Profile, store state.Store, image ref.Reference, timeout time.Duration, concurrency int, platform string) (probe.Result, []probe.Result, int) {
	profile, ok := appconfig.FindProfile(profiles, image.Registry)
	if !ok {
		result := probe.Image(ctx, &http.Client{Timeout: timeout}, image.String(), "source", platform)
		if result.OK {
			return result, []probe.Result{result}, exitOK
		}
		return probe.Result{}, []probe.Result{result}, exitNoUsableMirror
	}
...
Usage:
  ...
  mip pull IMAGE [--engine docker|podman|nerdctl] [--dry-run] [--platform PLATFORM] [--no-verify-digest] [--json]
